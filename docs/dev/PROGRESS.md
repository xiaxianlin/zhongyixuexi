...
| S9.8 | done | Web 前端:**决策(用户选定)——另起独立 `web/` 项目,不改造现有 `src/`**,桌面版(Phase 0-8)完全不受影响。新增第四个 TS root `tsconfig.webapp.json`(`web/src/**`,复用根 `package.json` 里已有的 react/react-dom/react-router-dom/vite/@vitejs/plugin-react 依赖,不必新建 package.json/node_modules);`vite.web.config.ts` 用 `root: 'web'` 独立跑;新增 `web:dev`/`web:build`/`web:preview` script,`typecheck:webapp` 并入根 `typecheck`,eslint 加 `web/src/**` 块(与桌面版 `src/**` 共用 react-hooks/react-refresh 规则)。**这不是"港移植",是照着后端 API 现状新建的一套界面**:登录/邀请码注册(`LoginPage`/`RegisterPage`)、书库+书籍详情(`LibraryPage`/`BookDetailPage`,读原文,S9.4 未迁移 `paragraph_analyses`/`ai_cache` 所以暂无 AI 解读面板,如实反映现状而非假装存在)、全文检索(`SearchPage`)、钱包余额+流水(`WalletPage`)、AI 问答(`ChatPage`,403 时提示充值而非裸报错)、管理后台(`AdminPage`:书/章/段 CRUD + 发布下线 + 邀请码生成作废 + 会员人工充值,一个页面内嵌数个 section)。鉴权:`web/src/lib/auth.tsx` 用 Context 存 token(localStorage 持久化),前端解码 JWT payload 仅用于"要不要显示某导航项"这类体验优化,**真正的权限判断在服务端**(`requireTier`/`requireAdmin`)。`web/src/lib/api.ts` 是唯一的 fetch 封装(带 Bearer header + 错误映射)。为让 UI 真正可用,顺手在后端补了 3 个此前没有的只读接口(否则管理后台没法用):`GET /admin/books`(公开的 `GET /books` 只返回 `published`,admin 需要看到 draft)、`GET /admin/books/:bookId`(`getBookDetail` 加 `includeUnpublished` 选项)、`GET /admin/users`(充值表单要选会员,原来没有任何列表接口)。**发现并修的一个真实 bug**:浏览器跨域请求被挡(Fastify 默认不处理 CORS 预检),装 `@fastify/cors`(`origin: true`,鉴权是 Bearer token 不是 cookie,反射来源没有 CSRF 风险),否则整个前端连不上后端。`npm run check` 全绿(tsc+lint+test,`web/` 无自动化测试,纯前端交互用真实浏览器验证,下面详述)。**真实浏览器端到端验证**(临时 Postgres + `npm run web:dev` + `mcp__Claude_Browser__*`):游客浏览书库/书籍详情/检索 → 邀请码注册自动登录 → 免费会员钱包显示余额 0、AI 问答页 403 时显示"需要充值会员"友好提示(不是裸错误) → 管理员登录后台创建书籍/发布/新增章节段落(即时刷新可见)、生成邀请码、给会员人工充值(表单提交后立刻显示"成功,最新余额:1000") → 充值会员钱包显示余额与流水 → 充值会员 AI 问答门槛通过(用假 DeepSeek key 验证到"确实发起了调用"这一步,真实调用链路已在 S9.7 用假 DeepSeek 服务器验证过,这里不重复烧真实 token)。**已知缺口,留给以后**:AI 解读展示(等 S9.4 遗留的 `paragraph_analyses` 迁移)、聊天历史跨刷新保留(当前只在页面内存里累积,刷新丢失,服务端数据不丢)、SRH-03 搜索结果跳转到具体段落(当前只跳书籍详情页顶部)、管理后台的"删除书籍/下线"等危险操作没有二次确认弹窗。**补充(用户要求"页面设计要沿用原来的设计")**:初版只做通了功能,视觉是浏览器默认样式,未套用桌面版的暖纸古籍风。补做:`web/src/styles/theme.css` 直接拷贝 `src/styles/theme.css`(暖纸/水墨/夜间三套 token,原样不改);新增 `web/src/lib/theme.tsx`(桌面版 `ui.ts` 主题机制的 React Context 版本——`data-theme` 属性 + localStorage,因为这个代码区没有引入 zustand,沿用本项目已有的 Context 模式即 `auth.tsx` 的写法);`web/src/index.css` 整份重写,对着 token 搭页面壳(`main.css` 的 `.app`/`.app__header`/`.app__nav`)、书库册页卡片网格(简化自 `library.css` 的 `.bookcard`,去掉拖拽排序/换封面这些不适用的交互)、原文阅读排版(宋体正文、宽行距)、以及桌面版没有的登录/注册/钱包/问答/后台页面的表单/表格/聊天气泡样式。`App.tsx` 加了主题切换按钮;`LibraryPage`/`LoginPage`/`RegisterPage`/`ChatPage`/`AdminPage` 只改 className,逻辑不动。三套主题 + 全部页面均已用真实浏览器过了一遍截图确认。**再补充(用户改口要求"完全重新设计",不再受桌面版约束)**:上面这版暖纸移植被推翻重做。新方案「青瓷诊脉」——色板换成青瓷绿(药罐釉色)为主调(桌面版是暖纸+朱砂),铜绿作界格辅线,朱砂收窄为只用于告警/扣费;版式从圆角卡片+投影改为仿雕版"界格"框线网格;明暗只留两档(跟随系统偏好,不再是桌面版的三档);数字/时间统一走系统等宽字体,做出"诊脉读数"对齐感。核心设计动作:`web/src/lib/dialogue.ts` 用一个纯函数识别段落原文里"曰：...\n然：..."的字面模式——《难经》本就是问答体(81 篇,篇篇一问一答),这不是外加的装饰,是原文结构本身;命中就渲染成带"曰/然"标签的问答块,不命中(如《伤寒论》等非问答体经典)原样按普通段落渲染,不强行拆分。落地前先出了一版 Artifact 视觉方案(色板+排版+关键页面 mockup)经用户过目确认方向,再回到 `web/` 里实现,避免返工。`ChatPage` 顺手改成两栏(对话 + 钱包余额侧栏,复用已有的 balance 状态,没加新接口);`LibraryPage` 补上了 API 早就返回但旧版没显示的 category 分类标签。`npm run check`(webapp 范围)全绿,三级权限/邀请码/钱包/AI 问答/管理后台全部页面在浏览器里过了一遍明暗两套主题的真实截图。 |
| S9.9 | doing | **在仓库范围内能做的部分已完成;实际"部署上线"本身超出本会话能力范围,见下方阻塞说明。** 已完成:①基础限流/防刷——装 `@fastify/rate-limit`,全局每 IP 300 次/分钟(`RATE_LIMIT_MAX` 可配),`/auth/register`/`/auth/login` 单独收紧到 10 次/分钟(防暴力破解邀请码/密码),`/chat` 单独收紧到 20 次/分钟(防止在余额耗尽前刷爆上游 DeepSeek 调用);本机验证连续 13 次登录请求在第 11 次开始返回 429,符合预期。②部署产物——`server/Dockerfile`(多阶段构建,构建上下文须是仓库根目录,新增 `tsconfig.server.build.json` 排除测试文件的产物构建),`web/Dockerfile` + `web/nginx.conf`(多阶段构建静态站点,SPA fallback 到 `index.html`),根目录 `docker-compose.yml`(postgres + server + web 三容器编排,密钥走 `.env` 文件,参照 `server/.env.example`),`.dockerignore`。`server/.env.example` 补全 S9.1-S9.9 陆续引入但从未回填的环境变量(`JWT_SECRET`/`ADMIN_BOOTSTRAP_*`/`DEEPSEEK_*`/`RATE_LIMIT_MAX`)。**已用本机 Docker 完整验证**:两个镜像各自构建成功;`docker compose up` 拉起三容器(postgres 健康检查通过后 server 才启动),容器内环境变量正确注入,`/health` 200,邀请码全流程可用的 `/auth/login`(bootstrap admin)在容器间通过 Docker 内网(`postgres:5432`)连接数据库成功返回 token,web 容器的 nginx 静态站点 200,验证后完整 `down -v` 清理(容器/网络/卷/镜像均已删除,未留残留)。**阻塞、需要用户提供,本会话无法自行完成**:③真实的香港服务器(需要用户提供服务器访问权限或自行在服务器上执行 `docker compose up`);④域名 + DNS 解析;⑤HTTPS 证书(依赖③④才能签发,例如 Let's Encrypt/Caddy 自动签发或 nginx+certbot);⑥监控——当前只有 Fastify 内置 pino 结构化日志,未接入外部监控栈(Prometheus/Grafana 等),对当前"邀请制小范围、不大规模开放"的量级判断为暂不需要,量级变化时再评估。 |
...
| S10.2 | done | 去掉 web 端明暗主题切换,彻底对齐"照搬桌面端"——桌面版本来就只有固定暖纸一套主题,从来没有切换器 UI。删掉整个 `web/src/lib/theme.tsx`(S9.8 补的 Context 版明暗切换,青瓷诊脉改版留下的唯一残余功能);`web/src/main.tsx` 去掉 `initTheme()` 调用,改成跟桌面版 `src/main.tsx` 一字不差的 `document.documentElement.dataset.theme = 'paper'`(含同一行注释);`web/src/App.tsx` 去掉 `ThemeProvider` 包裹层、`Nav()` 里的 `useTheme()` 调用、以及明暗切换 `<button>`。`grep -rn "app__themeBtn"` 确认该 class 在桌面版 `src/` 里从未被使用,只有 web 端引用,故移除按钮不影响桌面;**有意不动** `shared/ui/main.css` 里的 `.app__themeBtn` 规则——它是 S10.1 从桌面版 `main.css` 原样迁移过来的既有代码(桌面版这条规则本来就是死代码,不是这次改动引入的),清理"非本次改动引入的共享文件死代码"超出本 slice 范围。`npm run check` 全绿(163 test 不变,仅剩 1 条 S9.8 就有的预先存在的 warning);本机 docker-compose 重新构建 web 镜像后真实浏览器截图确认:导航栏切换按钮已消失,其余布局/配色无回归。 |
| S10.3 | done | 共享 `AiSubCode` + 中立 `AiError`,顺带完成 S10.1 延后的 `tsconfig.server.json` rootDir 改造。新建 `shared/core/ai/errors.ts`:`AiSubCode`(11 值,取桌面版原有集合——服务端原来的 7 值本就是桌面版 11 值的子集,不存在需要新增的值)+ 中立 `AiError extends Error` 类(不耦合 `AppError`/Fastify)。`electron/ai/errors.ts` **保留原有公开接口不变**(`aiError`/`isAiError`/`aiSubCode`/`AiErrorDetails` 全部原地不动,消费方 `electron/ai/deepseek.ts`/`electron/services/ai.ts` 零改动),只把 `AiSubCode` 类型声明换成从 `@shared/core/ai/errors` 导入再 re-export。`server/src/ai/errors.ts` 整个替换成一行 re-export(`export { AiError, type AiSubCode } from '../../../shared/core/ai/errors'`,**用相对路径,不用 `@shared` 别名**——原因见方案文档:`server:build` 是纯 `tsc` CommonJS emit,不会重写别名),消费方 `deepseek.ts`/`routes/chat.ts` 同样零改动。**顺手发现并修的第三份重复**:`src/models/ai/types.ts` 里还有一份手抄的 `AiSubCode`(注释写着"mirrored from electron/ai/errors.ts"),虽然不在方案的迁移表里,但既然这一 slice 就是消灭 `AiSubCode` 重复,留着第三份不合逻辑,顺带把它也换成从 shared 导入(`toDegradedReason`/`DegradedReason`/`DEGRADED_COPY` 等渲染层专属逻辑不动)。**触发了一次真实的编译期漂移检测**:`server/src/routes/chat.ts` 的 `AI_ERROR_MESSAGES: Record<AiSubCode, string>` 是穷举式对象字面量,`AiSubCode` 一从 7 值扩到 11 值就直接 `tsc` 报错缺 4 个 key——这正是"真正共享类型"该有的效果(以前两边手抄不会有任何报错提示已经漂移)。照桌面版 `toDegradedReason` 的现有分组(`AI_KEY_NOT_CONFIGURED`/`AI_STORAGE_UNAVAILABLE`/`AI_PROVIDER_NOT_CONFIGURED` 同归"未配置"一类)补齐 4 条文案,这 4 个新 sub-code 目前服务端还没有任何代码会真正抛出(是为 S10.4/S10.5 之后共享 guard.ts/deepseek.ts 预留的),本 slice 不改变任何现有可达行为。(S10.4 备注:`AI_GUARD_BLOCKED` 目前仍未被 server 端任何代码实际抛出——`chat-service.ts` 的 guard 命中走的是 `shouldBlock()` 返回值分支,不走异常路径,该 sub-code 暂时仍是"为将来预留"。)**`tsconfig.server.json` rootDir 改造**(S10.1 延后到本 slice 的部分):`rootDir` 从 `"server"` 改成 `"."`,`include` 加 `"shared/**/*.ts"`(不加 `paths`,服务端就该用相对路径);`tsconfig.server.build.json` 的 `exclude` 同步加 `shared/**/*.test.ts`。**级联改动**(rootDir 从 `.` 起算后 emit 目录结构从 `out-server/src/...` 变成 `out-server/server/src/...`,`shared/` 部分则落在 `out-server/shared/...`,已用真实 `tsc` 构建产物确认):`package.json` 的 `server:start` 改成 `node out-server/server/src/index.js`;`server/Dockerfile` 加 `COPY shared ./shared`(否则容器内 `tsc` 找不到 shared 源码,重演 S10.1 web 端踩过的同一类坑)+ `CMD` 路径同步改成 `out-server/server/src/index.js`。`npm run check` 全绿。**额外做了 tsc --noEmit 覆盖不到的运行时验证**(方案文档明确要求):本机 `npm run server:build` 后直接 `node -e` 加载编译产物构造 `AiError` 确认 CommonJS `require('../../../shared/core/ai/errors')` 相对路径在真实 Node 运行时能解析;再用本机 docker-compose 重新构建 server 镜像、启动容器,`curl /health` 返回 `200 {"status":"ok"}`,容器日志确认服务正常监听,证明新 rootDir/emit 路径在容器环境下同样可运行。 |
| S10.4 | done | 共享 `guard.ts`(三层红线守卫的第 2/3 层)+ `RED_LINE_PROMPT`(第 1 层)。逐字节比对确认 `electron/ai/guard.ts`/`server/src/ai/guard.ts` 逻辑完全相同(仅顶部注释措辞不同),两边各自的 `guard.test.ts` 也是同一组 11 个测试用例的重复(仅助手函数写法略有差异)——这次是真正的"纯移动",不像 S10.3 的 `errors.ts` 那样两边有平台专属的包装逻辑要保留,所以直接删除两份原文件+两份重复测试,新建 `shared/core/ai/guard.ts` + `shared/core/ai/guard.test.ts`(11 个用例保留 1 份,以内容更完整的桌面版为底本)。`server/src/ai/chat-service.ts` 的导入改成相对路径 `'../../../shared/core/ai/guard'`。`RED_LINE_PROMPT` 常量同样字节相同,挪到新建的 `shared/core/ai/red-line.ts`;但两边的 prompt 构建函数不同(`buildModernPrompt` 桌面版 AI-01 专属,`buildQaPrompt` 服务端 AI-05 专属),所以**不**整体搬移 `prompts.ts`,只让两边的 `prompts.ts` 改为从 shared 导入 `RED_LINE_PROMPT` 再 `export` 回去(消费方 `prompts.test.ts` 两边都不用改,因为它们都是从本地 `./prompts` 导入,导入路径没变)。**发现并修复一个真实的基础设施缺口**:`vitest.config.ts` 此前完全没有 `@shared` 别名配置(和其余 4 个消费方 tsconfig/vite 配置不一样)。S10.3 因为 `electron/ai/errors.ts` 用的是 `import type { AiSubCode } from '@shared/core/ai/errors'`——纯类型导入在编译期会被完全擦除,运行时根本不会有 `require()` 调用,所以侥幸没暴露这个洞;但本 slice 的 `RED_LINE_PROMPT` 是运行时常量(值导入),Vitest 用 Vite 的 resolver 加载测试时真的要去 resolve `@shared/...`,直接报 "Cannot find package"。修法:给 `vitest.config.ts` 加 `resolve.alias: { '@shared': resolve(__dirname, 'shared') }`(只加 `@shared`,不加 `@`——当前没有任何测试通过 `@/*` 做值导入,不做投机性的提前修复)。这个坑意味着:S10.1-S10.3 的"`npm run check` 全绿"并不能真正保证所有后续通过 `@shared` 做值共享的 slice 都没问题,本 slice 把这个洞堵上后,S10.5 起的所有 slice 才能安全依赖"测试绿=真的没问题"。`npm run check` 全绿(25 个测试文件、152 个测试——比 S10.3 时少 11 个,精确对应被消灭的 11 个重复 guard 测试,新 shared 版本的 11 个测试原样保留,零覆盖率损失)。**server 端运行时验证**:`npm run server:build` 后直接 `require()` 编译产物验证 `shouldBlock`/`sanitizeOutput`/`RED_LINE_PROMPT`/`buildQaPrompt` 在真实 Node CommonJS 运行时都能正确加载和工作(`buildQaPrompt` 生成的 system message 确实包含红线文本)。 |
